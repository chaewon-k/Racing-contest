...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        way_points = []
        for i in range(9):
            y = math.sqrt(math.pow(sensing_info.distance_to_way_points[i],2)-math.pow(sensing_info.to_middle,2))
            way_points.append({'x':0, 'y':y})

        for i in range(9):
            x = way_points[i]['x'] + (10*math.sin(math.radians(sensing_info.track_forward_angles[i])))
            y = way_points[i]['y'] + (10*math.sin(math.radians(sensing_info.track_forward_angles[i])))
            way_points.append({'x':x,'y':y})
            
        ref_idx = max(1,min(9,int(sensing_info.speed/25)))
        target_x = way_points[ref_idx+2]['x']
        target_y = way_points[ref_idx+2]['y']

        set_steering = (math.atan((target_x-sensing_info.to_middle)/target_y)-np.radians(sensing_info.moving_angle)) * 0.8
