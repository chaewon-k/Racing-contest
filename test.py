...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        plus_steering_at_corner = 0
        obstalce_pop  = 0
        if avoid_check and sensing_info.track_forward_obstacles:
            x = sensing_info.track_forward_obstacles[0]['dist']
            x_int = int(x//10)
            for i in range(x_int-1, x_int+2):
                if sensing_info.track_forward_angles[i] > 30:
                    sensing_info.track_forward_obstacles.pop(0)
                    obstacle_pop = 1
                    if x < 0: plus_steering_at_corner = 0.15
                    else: plus_steering_at_corner = -0.15
                    print('홀롤로')
                    break
                elif sensing_info.track_forward_angles[i] < -30:
                    sensing_info.track_forward_obstacles.pop(0)
                    obstacle_pop = 1
                    if x < 0: plus_steering_at_corner = -0.15
                    else: plus_steering_at_corner = 0.15
        ...
        if plus_steering_at_corner and  obstalce_pop:
            set_steering += plus_steering_at_corner
            print('후유유유유유융유유유유')
