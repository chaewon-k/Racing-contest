...
class DrivingClient(DrivingController):
    ...
    def obstacle_avoid(self, sensing_info):
        ...
        if len(forward_obstacles) > 0:
            ...
            for i in available_pos:
                ...
                if not check:
                    last_dist = i
                    angle = (math.atan((i - sensing_info.to_middle) / target_y) - math.radians(sensing_info.moving_angle))
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        # if sensing_info.speed > 150: steer_factor = sensing_info.speed * 0.7
        ...
        #and sensing_info.to_middle-4<sensing_info.track_forward_obstacles[0]['to_middle']<sensing_info.to_middle+4
        avoid_check = 0
        if sensing_info.track_forward_obstacles:
            for i in sensing_info.track_forward_obstacles:
                if sensing_info.to_middle-2.5<i['to_middle']<sensing_info.to_middle+2.5 and i['dist']<angle_num * 10:
                    avoid_check = 1

        if sensing_info.track_forward_obstacles and 0 < sensing_info.track_forward_obstacles[0]['dist'] < (
        angle_num) * 10 and avoid_check:
            ...
            if not avoid_steering:
                ...
                if sensing_info.speed < 120: set_steering *= 1.2
                if sensing_info.speed >= 120: set_steering *= 0.9
                if sensing_info.speed > 150: set_steering *= 0.8
        else:
