...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if self.reverse_drive and self.reverse_drive_count < 25:
            if self.reverse_steering != 0:
                ...
                if sensing_info.to_middle >= 0:
                    ...
                    set_steering = -1
            print(set_steering, self.reverse_steering)
        ...
        if self.reverse_drive_count == 25:
            self.reverse_drive_count = 0
            self.reverse_drive = 0
        ...
        angle_num) * 10 :
            ...
            if not avoid_steering:
            ...
            elif self.last_steering >= 0 and -20<=sensing_info.moving_angle <=20 and avoid_steering[2] and len(avoid_steering[2])>=5 and avoid_check:
                set_steering = avoid_steering[2][0]
            elif self.last_steering < 0 and -20<=sensing_info.moving_angle <=20 and avoid_steering[1] and len(avoid_steering[1])>=5 and avoid_check:
                set_steering = avoid_steering[1][-1]
                ...
                if set_steering > 0:
                    ...
                    if avoid_steering[0] > 0:
                        ...
                        if avoid_steering[2] and math.radians(ref_angle + sensing_info.moving_angle) > \
                                avoid_steering[2][0] and len(avoid_steering[2]) >= 5:
                            set_steering += avoid_steering[0] * 0.6
                        else:
                    ...
                    if avoid_steering[0] < 0:
                        ...
                        if avoid_steering[1] and avoid_steering[1][-1] > math.radians(
                                ref_angle + sensing_info.moving_angle) and len(avoid_steering[1]) >= 5:
                            set_steering -= avoid_steering[0] * 0.6
                        else:  # 못가는상황
                            set_steering = avoid_steering[0]
        ...
        if emergency_brake:
            if set_steering > 0:
                ...
                set_steering -= 0.4
                set_throttle = 0.8
        self.last_steering = set_steering
        ...
        ######
        if abs(sensing_info.to_middle) > half_load_width + 1:
        ...
        if self.reverse_complete and self.reverse_complete_cnt < 10:
            set_steering = -self.reverse_steering
            self.reverse_complete_cnt += 1
        if self.reverse_complete_cnt ==10:
            self.reverse_complete = 0
            self.reverse_complete_cnt = 0
            self.reverse_steering = 0
