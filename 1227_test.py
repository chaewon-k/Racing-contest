...
class DrivingClient(DrivingController):
    ...
    def obstacle_avoid(self, sensing_info):
        ...
        for obstacle in obstacle_list:
            if obstacle['dist'] >= 0 and obstacle['dist'] < max_dist and abs(obstacle['to_middle']) <= (half_load_width + 1.0):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if sensing_info.track_forward_obstacles and sensing_info.to_middle - 2 < sensing_info.track_forward_obstacles[0]['to_middle'] < sensing_info.to_middle + 2:
            for i in sensing_info.track_forward_obstacles:
                if i['dist'] < angle_num * 10:

                    avoid_check = 1
                    break
        elif sensing_info.track_forward_obstacles and sensing_info.to_middle <= 0 and sensing_info.to_middle < sensing_info.track_forward_obstacles[0]['to_middle'] < 0:avoid_check = 1
        elif sensing_info.track_forward_obstacles and sensing_info.to_middle >= 0 and sensing_info.to_middle > sensing_info.track_forward_obstacles[0]['to_middle'] > 0:avoid_check = 1
        ...
        if avoid_check and 0 < sensing_info.track_forward_obstacles[0]['dist'] < (angle_num) * 10 :
            ...
            if not avoid_steering:
                ...
                if set_steering > 0:
                    ...
                    if avoid_steering[0] > 0:
                        ...
                        if avoid_steering[3] == 1 and avoid_steering[2] and math.radians(ref_angle - sensing_info.moving_angle) > avoid_steering[2][0] and len(avoid_steering[2]) >= 5:
                    ...
                    if avoid_steering[0] < 0:
                        ...
                        if avoid_steering[3] < 2 and avoid_steering[1] and avoid_steering[1][-1] > math.radians(ref_angle - sensing_info.moving_angle) and len(avoid_steering[1]) >= 5:
                            set_steering -= avoid_steering[0] * 0.5         #
                        else:  # 못가는상황
                            set_steering = avoid_steering[0]
