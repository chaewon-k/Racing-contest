...
class DrivingClient(DrivingController):
    ...
    def obstacle_avoid(self, sensing_info):
        ...
        for opponent in sensing_info.opponent_cars_info:
            if sensing_info.lap_progress > 0.5 and -10<opponent['dist'] < 20 and abs(opponent['to_middle'])<=(half_load_width +1.0):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        # =========================================================== #
        # Editing area starts from here
        set_steering = car_controls.steering
        set_brake = 0
        ...
        avoid_check = 0
        # if sensing_info.track_forward_obstacles and sensing_info.track_forward_obstacles[0]['dist'] > (angle_num - 3) * 10:
        #     set_steering += middle_add * 0.7

        # if sensing_info.opponent_cars_info:
        #     for opponent in sensing_info.opponent_cars_info:
        #         if sensing_info.lab_progress > 0.5 and 0<=opponent['dist']<30 and opponent['speed'] <= sensing_info.speed:
        #             sensing_info.track_forward_obstacles.append(opponent)

        if sensing_info.track_forward_obstacles and sensing_info.to_middle - 2 < i['to_middle'] < sensing_info.to_middle + 2:
            for i in sensing_info.track_forward_obstacles:
                if i['dist'] < angle_num * 10:
                    avoid_check = 1
                    break
        ...
        if avoid_check and 0 < sensing_info.track_forward_obstacles[0]['dist'] < (angle_num) * 10 or (sensing_info.opponents_cars_info):
            ...
            if not avoid_steering:
                ...
                if abs(sum(sensing_info.track_forward_angles)) >= 300:
                    print(sum(sensing_info.track_forward_angles))
                    set_steering += middle_add * 0.8
                ###
                ...
                if set_steering > 0:
                    ...
                    if avoid_steering[0] > 0:
                        ...
                        if avoid_steering[2] and math.radians(ref_angle - sensing_info.moving_angle) > avoid_steering[2][0] and len(avoid_steering[2]) >= 5:
                            set_steering += avoid_steering[0] * 0.5         #
                        else:
                    ...
                    if avoid_steering[0] < 0:
                        ...
                        if avoid_steering[1] and avoid_steering[1][-1] > math.radians(ref_angle - sensing_info.moving_angle) and len(avoid_steering[1]) >= 5:
                            set_steering -= avoid_steering[0] * 0.5         #
                        else:  # 못가는상황
                            set_steering = avoid_steering[0]
