...
class DrivingClient(DrivingController):
    ...
    def obstacle_avoid(self, sensing_info):
        obstacles_list = sensing_info.track_forward_obstacles
        ...
        if len(forward_obstacles) > 0:
            ...
            set_steering = (np.arctan((target_x - sensing_info.to_middle) / target_y) - np.radians(
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        # ======================= set steering ============================ #
        way_points = []
        print(sensing_info.distance_to_way_points)
        ...
        ref_idx = max(1, min(9, int(sensing_info.speed / 30)))
        ref_angle = sensing_info.track_forward_angles[ref_idx]
        target_x = way_points[ref_idx]['x']
        target_y = way_points[ref_idx]['y']

        ### !! 속도에 따라 조절해야함.
        ### !! 전방의 커브가 완만한지, 급격한지에 따라 조절해야함
        steer_factor = 1
        if sensing_info.speed > 120: steer_factor = sensing_info.speed * 0.01
        if sensing_info.speed > 140: steer_factor = sensing_info.speed * 0.015
        # print(steer_factor)

        set_steering = (
                    math.atan((target_x - sensing_info.to_middle) / target_y) - math.radians(sensing_info.moving_angle))

        if steer_factor != 0:
            set_steering /= steer_factor
        ...
        set_throttle = 0.9 + throttle_factor

        if sensing_info.speed < 120: set_throttle = 1
        if sensing_info.speed > 120: set_throttle = 0.8
        if sensing_info.speed > 160: set_throttle = 0.6
        if sensing_info.speed > 180: set_throttle = 0
